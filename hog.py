...
def pigs_on_prime(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    if (is_prime(player_score)): #& player_score != 1
        #prime = player_score
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while max(score0, score1) < goal:
        if who == 0:
            current_score = take_turn(strategy0(score0, score1), score0, score1, dice, goal)
            score0 += current_score + pigs_on_prime(score0 + current_score, score1)
        
        else:
            current_score = take_turn(strategy1(score1, score0), score1, score0, dice, goal)
            score1 += current_score + pigs_on_prime(score1 + current_score, score0)
        who = next_player(who)
    # END PROBLEM 5
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if (last_leader == leader_new) or (leader_new == None):
        return leader_new, None
    elif (last_leader != leader_new):
        message = "Player " + str(leader_new) + " takes the lead by " + str(abs(score0 - score1))
        return leader_new, message
    # END PROBLEM 6
...
def oink_points_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    call = oink_points(score, opponent_score) 
    if call < threshold:
        return num_rolls 
    else: 
       return 0
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    oink_s = oink_points(score, opponent_score) 
    s_oink = score + oink_s
    if is_prime(s_oink) or oink_points_strategy(score, opponent_score, threshold, num_rolls) == 0:
        return 0 
    else:
        return num_rolls 
    # END PROBLEM 11
